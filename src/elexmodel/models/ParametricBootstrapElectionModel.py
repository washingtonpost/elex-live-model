...
class BootstrapElectionModel(BaseElectionModel):
    ...
    def compute_bootstrap_errors(self, reporting_units, nonreporting_units, unexpected_units):
        ...
        x_train = self.featurizer.featurize_fitting_data(reporting_units, add_intercept=self.add_intercept, center_features=False)
        ...
        md_y = smf.mixedlm(f"normalized_margin ~ baseline_normalized_margin + {fixed_effect_string}", df_train, groups=reporting_units["postal_code"])
        md_z = smf.mixedlm(f"turnout_factor ~ baseline_normalized_margin + {fixed_effect_string}", df_train, groups=reporting_units["postal_code"])
        mdf_y = md_y.fit()
        mdf_z = md_z.fit()
        ...
        aggregate_indicator = pd.get_dummies(pd.concat([reporting_units, nonreporting_units, unexpected_units], axis=0)['postal_code'])
        ...
        for b in range(self.B):
            ...
            md_y_b = smf.mixedlm(f"normalized_margin ~ baseline_normalized_margin + {fixed_effect_string}", df_train_B, groups=reporting_units["postal_code"])
            md_z_b = smf.mixedlm(f"turnout_factor ~ baseline_normalized_margin + {fixed_effect_string}", df_train_B, groups=reporting_units["postal_code"])
            mdf_y_b = md_y_b.fit()
            mdf_z_b = md_z_b.fit()
            
            y_test_pred_B[:,b] = mdf_y_b.predict(df_test).values + (aggregate_indicator_test.values @ epsilon_y_hat).flatten()
            z_test_pred_B[:,b] = mdf_z_b.predict(df_test).values + (aggregate_indicator_test.values @ epsilon_y_hat).flatten()
        ...
        test_epsilon = self.rng.multivariate_normal([0, 0], cov_mat_eps, size=(len(states_that_need_random_effect), self.B))
        test_epsilon_y = test_epsilon[:,:,0]
        test_epsilon_z = test_epsilon[:,:,1]
 
        test_residuals_y = test_delta_y + aggregate_indicator_test.values[:,states_that_need_random_effect] @ test_epsilon_y
        test_residuals_z = test_delta_z + aggregate_indicator_test.values[:,states_that_need_random_effect] @ test_epsilon_z
