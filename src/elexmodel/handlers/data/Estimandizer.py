...
def margin(data_df):
    data_df["weights"] = data_df[f"{BASELINE_PREFIX}dem"] + data_df[f"{BASELINE_PREFIX}gop"]
    
    if f"{BASELINE_PREFIX}margin" not in data_df.column:
        data_df[f"{RESULTS_PREFIX}margin"] = data_df[f"{RESULTS_PREFIX}dem"] - data_df[f"{RESULTS_PREFIX}gop"]
        data_df[f"{RESULTS_PREFIX}normalized_margin"] = np.nan_to_num(
            (data_df[f"{RESULTS_PREFIX}dem"] - data_df[f"{RESULTS_PREFIX}gop"]) / (data_df[f"{RESULTS_PREFIX}dem"] + data_df[f"{RESULTS_PREFIX}gop"])
        )
    else:
        data_df["weights"] = data_df[f"{BASELINE_PREFIX}dem"] + data_df[f"{BASELINE_PREFIX}gop"]
        data_df[f"{BASELINE_PREFIX}margin"] = data_df[f"{BASELINE_PREFIX}dem"] - data_df[f"{BASELINE_PREFIX}gop"]
        data_df[f"{RESULTS_PREFIX}normalized_margin"] = np.nan_to_num(
            (data_df[f"{BASELINE_PREFIX}dem"] - data_df[f"{BASELINE_PREFIX}gop"]) / data_df["weights"]
        )