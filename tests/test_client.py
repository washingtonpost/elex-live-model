...
def test_winsorize_intervals(model_client, va_governor_county_data, va_config):
    ...
    for index, value in enumerate(winsorize_data_unit["lower_0.9_turnout"].values):
        ...
        if value < non_value:
            print("LOWER: " + str(value) + " is less than " + str(non_value) + " at " + str(index))
    ...
    for index, value in enumerate(winsorize_data_unit["upper_0.9_turnout"].values):
        ...
        if value > non_value:
            print("UPPER: " + str(value) + " is greater than " + str(non_value) + " at " + str(index))

    assert (
        winsorize_data_state["lower_0.9_turnout"].values[0] >= non_winsorize_data_state["lower_0.9_turnout"].values[0]
    )
    assert (
        winsorize_data_state["upper_0.9_turnout"].values[0] <= non_winsorize_data_state["upper_0.9_turnout"].values[0]
    )
