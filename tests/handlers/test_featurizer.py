...
def compute_testing_mean_for_centering():
    ...
    assert featurizer.column_means.equals(pd.Series({'a': 1.5, 'b': 2.5, 'c': 3.5}, index=["a", "b", "c"], dtype=np.float64))
...
def test_generate_fixed_effects_not_all_reporting(va_governor_county_data):
    ...
    assert "county_fips_51003" in nonreporting_data_features.columns  # in here because manaully added (since in reporting_units)

    assert "county_fips_51790" not in reporting_data_features.columns  # not in here because not reporting
    assert "county_fips_51790" not in nonreporting_data_features.columns  # not in here because not in featurizer.complete_features
...
def test_generate_fixed_effects_mixed_reporting(va_governor_precinct_data):
    ...
    assert "county_fips_51003" in reporting_data_features.columns  # in here because get_dummies
    assert "county_fips_51003" in nonreporting_data_features.columns  # in here because manaully added (because in reporting_data)

    assert "county_fips_51013" in reporting_data_features.columns  # in here because get_dummies
    assert "county_fips_51013" in nonreporting_data_features.columns  # in here because get_dummies (and in featurizer.complete_features)
